# vendored from VisoMaster-fusion
"""
...
class _RMSNormMax(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if x.dtype == torch.float16 and _TRITON_RMSNORMMAX is not None and not self._triton_disabled:
...
class _UpBlock(nn.Module):
    ...
    def __init__(self, in_ch: int, out_ch: int) -> None:
        ...
        self.ct = nn.ConvTranspose2d(
            in_ch, out_ch, 3, stride=2, padding=0, bias=False
        )
