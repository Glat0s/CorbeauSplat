...
class ResetDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        desc_light.setStyleSheet(
            "color: #7f8c8d; font-size: 12px; margin-top: -15px; margin-left: 20px;"
        )
...
class ConfigTab(QWidget):
    ...
    def init_ui(self):
        ...
        self.lbl_workflow.setStyleSheet(
            "background-color: #1a3a5c; color: #7ec8e3; "
            "border: 1px solid #2a5a8c; border-radius: 6px; "
            "padding: 8px 12px; font-size: 12px; margin: 4px 0px;"
        )
        ...
        self.btn_delete_dataset.setStyleSheet(
            "background-color: #aa4444; color: white; border: none; padding: 5px;"
        )
        ...
        self.btn_quit.setStyleSheet(
            "QPushButton { border: none; color: #888888; font-size: 10px; } QPushButton:hover { color: #ff5555; }"
        )
        ...
        self.btn_relaunch.setStyleSheet(
            "QPushButton { border: none; color: #888888; font-size: 10px; } QPushButton:hover { color: #ffffff; }"
        )
        ...
        self.btn_reset.setStyleSheet(
            "QPushButton { border: none; color: #884444; font-size: 10px; font-weight: bold; } QPushButton:hover { color: #ff0000; }"
        )
    ...
    def update_ui_state(self):
        ...
        is_video = (mode == "gsplat" and self.radio_video.isChecked()) or (
            mode in ["360", "4dgs", "vr180"]
        )
    ...
    def browse_input(self):
        ...
        if mode == "gsplat":
            ...
            if self.radio_source_folder.isChecked():
                ...
                paths, _ = get_open_file_names(self, tr("group_input"), "", filters)
                if paths:
            ...
            if self.radio_source_folder.isChecked():
                ...
                paths, _ = get_open_file_names(
                    self, tr("group_input"), "", "Images (*.jpg *.jpeg *.png);;Tous (*.*)"
                )
                if paths:
                    self.input_path.setText(
                        paths[0]
                    )  # Enforce single image for sharp if not folder
            ...
            if paths:
                if len(paths) > 1:
                    QMessageBox.warning(
                        self,
                        tr("msg_warning"),
                        tr("err_360_single_video", "Le mode 360 ne supporte qu'une vidéo."),
                    )
    ...
    def get_undistort(self):
        return self.undistort_check.isChecked()

    def set_undistort(self, val):
        self.undistort_check.setChecked(val)

    def get_auto_brush(self):
        return self.chk_auto_brush.isChecked()

    def set_auto_brush(self, val):
        self.chk_auto_brush.setChecked(val)

    def get_upscale(self):
        return self.chk_upscale.isChecked()

    def set_upscale(self, val):
        self.chk_upscale.setChecked(val)
    ...
    def set_processing_state(self, processing=True):
        ...
        if processing:
            self.btn_process.setStyleSheet(
                "font-size: 16px; font-weight: bold; background-color: #aaaaaa; color: white;"
            )
        else:
            self.btn_process.setStyleSheet(
                "font-size: 16px; font-weight: bold; background-color: #2a82da; color: white;"
    ...
    def retranslate_ui(self):
        ...
        self.btn_process.setText(
            tr("btn_process") if self.btn_process.isEnabled() else tr("btn_stop")
        )
