...
def bench_chroma_key(runs: int, warmup: int, device: str) -> list[dict]:
    ...
    try:
        ...
        if device == "cuda":
            ...
            results.append(
                {"name": "ChromaKey GPU (PyTorch+kornia)", "mean_ms": mean, "std_ms": std}
            )
            ...
            results.append(
                {
                    "name": "ChromaKey GPU batch=16 (per-frame)",
                    "mean_ms": mean / 16,
                    "std_ms": std / 16,
                }
            )
...
def bench_esrgan(runs: int, warmup: int, device: str) -> list[dict]:
    ...
    if not model_path.exists():
        results.append(
            {
                "name": "ESRGAN (weights missing)",
                "mean_ms": -1,
                "std_ms": 0,
                "notes": "download first",
            }
        )
        return results
    ...
    try:
        ...
        if esrgan_base.load(model_path):
            ...
            mean, std = timeit(lambda: esrgan_base.upscale_image(dummy), runs, warmup)
            results.append(
                {"name": "ESRGAN FP16+compile+CUDA kernels", "mean_ms": mean, "std_ms": std}
            )
            esrgan_base.unload()
        ...
        try:
            ...
            esrgan_ort = OptimizedRealESRGAN(
                model_path=model_path, device=device, use_fp16=False, use_compile=False
            )
            ...
            results.append(
                {"name": "ESRGAN ORT TRT EP", "mean_ms": -1, "std_ms": 0, "notes": str(e)}
            )
    except Exception as e:
...
def bench_sam(runs: int, warmup: int, device: str) -> list[dict]:
    ...
    try:
        ...
        sam_base = PersistentSAMPredictor(
            str(ckpt), "vit_b", device=device, use_fp16=True, use_compile=False
        )
        ...
        sam_graph = PersistentSAMPredictor(
            str(ckpt), "vit_b", device=device, use_fp16=True, use_compile=False
        )
        if sam_graph.is_loaded:
            ...
            results.append(
                {
                    "name": "SAM vit_b CUDA graph (encoder)",
                    "mean_ms": mean,
                    "std_ms": std,
            ...
            if hasattr(sam_graph._sam, "image_encoder"):
                try:
                    ...
                    mean, std = timeit(
                        lambda: sam_graph.predict_frame(dummy_rgb), runs // 2, sam_warmup
                    )
                    results.append(
                        {"name": "SAM vit_b +Flash+LN+WindowOps", "mean_ms": mean, "std_ms": std}
                    )
                except Exception as e:
                    results.append(
                        {
                            "name": "SAM vit_b +Flash+LN+WindowOps",
                            "mean_ms": -1,
                            "std_ms": 0,
                            "notes": str(e),
                        }
...
def bench_gfpgan(runs: int, warmup: int, device: str) -> list[dict]:
    ...
    if not ckpt.exists():
        results.append(
            {
                "name": "GFPGAN (weights missing)",
                "mean_ms": -1,
                "std_ms": 0,
                "notes": "auto-downloads on use",
            }
        )
        return results
    ...
    try:
        ...
        for use_graph, label in [(False, "Tier 2"), (True, "Tier 3")]:
            g = GFPGANEngine(
                checkpoint=ckpt, device=device, use_fp16=True, use_cuda_graph=use_graph
            )
            if g.load():
                mean, std = timeit(lambda _g=g, _d=dummy: _g.enhance_frame(_d), runs, warmup)
...
def bench_triton_layernorm(runs: int, warmup: int, device: str) -> list[dict]:
    ...
    if device != "cuda":
        return results
    try:
        from app.core.cuda_ext import ext_available, layer_norm_cuda
        ...
        for dim in (768, 1024, 1280):
            ...
            mean, std = timeit(lambda _m=ln_pt, _x=x: _m(_x), runs, warmup)
            ...
            mean, std = timeit(lambda _m=ln_tri, _x=x: _m(_x), runs, warmup)
            ...
            if ext_available():
                ...
                mean, std = timeit(
                    lambda _x=x, _w=w, _b=b: layer_norm_cuda(_x, _w, _b, 1e-6, False), runs, warmup
                )
                results.append(
                    {"name": f"LayerNorm-{dim} CUDA ext", "mean_ms": mean, "std_ms": std}
                )
            ...
            mean, std = timeit(lambda _m=ln_tri_gelu, _x=x: _m(_x), runs, warmup)
            results.append(
                {"name": f"LayerNorm+GELU-{dim} Triton fused", "mean_ms": mean, "std_ms": std}
            )

            if ext_available():
                mean, std = timeit(
                    lambda _x=x, _w=w, _b=b: layer_norm_cuda(_x, _w, _b, 1e-6, True), runs, warmup
                )
                results.append(
                    {"name": f"LayerNorm+GELU-{dim} CUDA ext fused", "mean_ms": mean, "std_ms": std}
                )

    except Exception as e:
        results.append(
            {"name": "Triton/CUDA LayerNorm", "mean_ms": -1, "std_ms": 0, "notes": str(e)}
        )
    return results
...
def bench_triton_window_ops(runs: int, warmup: int, device: str) -> list[dict]:
    ...
    if device != "cuda":
        return results
    try:
        from app.core.cuda_ext import ext_available, window_partition_cuda, window_unpartition_cuda
        ...
        if ext_available():
            ...
            results.append(
                {"name": "Window partition CUDA ext (float4)", "mean_ms": mean, "std_ms": std}
            )

            wins, pad_hw = window_partition_cuda(x, ws)
            hw_orig = (H, W)
            mean, std = timeit(
                lambda: window_unpartition_cuda(wins, ws, pad_hw, hw_orig), runs, warmup
            )
            results.append(
                {"name": "Window unpartition CUDA ext (float4)", "mean_ms": mean, "std_ms": std}
            )

    except Exception as e:
        results.append(
            {"name": "Triton/CUDA window ops", "mean_ms": -1, "std_ms": 0, "notes": str(e)}
        )
    return results
...
def bench_triton_esrgan_ops(runs: int, warmup: int, device: str) -> list[dict]:
    ...
    if device != "cuda":
        return results
    try:
        from app.core.cuda_ext import (
            ext_available,
            leaky_relu_scale_add_cuda,
            pixel_shuffle_2x_cuda,
        )
        from app.core.vendor.esrgan_triton_kernels import (
            MemEfficientDenseBlock,
            triton_pixel_shuffle_2x,
            triton_scale_add,
        )
        ...
        if ext_available():
            ...
            results.append(
                {"name": "LeakyReLU+scale_add CUDA ext (fused)", "mean_ms": mean, "std_ms": std}
            )
        ...
        results.append(
            {"name": "DenseBlock MemEfficient (Triton+CUDA)", "mean_ms": mean, "std_ms": std}
        )

    except Exception as e:
        results.append(
            {"name": "Triton/CUDA ESRGAN ops", "mean_ms": -1, "std_ms": 0, "notes": str(e)}
        )
    return results
...
def main():
    ...
    sections = {
        "Chroma Key (1920x1080)": _run(
            lambda: bench_chroma_key(args.runs, args.warmup, args.device)
        ),
        "RealESRGAN (256x256 -> 1024x1024)": _run(
            lambda: bench_esrgan(args.runs, args.warmup, args.device)
        ),
        "GFPGAN (512x512 face)": _run(lambda: bench_gfpgan(args.runs, args.warmup, args.device)),
        "Micro: LayerNorm (4096 tokens)": _run(
            lambda: bench_triton_layernorm(args.runs, args.warmup, args.device)
        ),
        "Micro: Window ops (56x56, ws=14)": _run(
            lambda: bench_triton_window_ops(args.runs, args.warmup, args.device)
        ),
        "Micro: ESRGAN ops": _run(
            lambda: bench_triton_esrgan_ops(args.runs, args.warmup, args.device)
        ),
    ...
    with open(out, "w", encoding="utf-8") as f:
        ...
        f.write(
            f"**Platform:** Windows 11 / RTX 4090 / CUDA 12.4 / PyTorch {torch.__version__}  \n"
        )
